...
schema_run_python_file = types.FunctionDeclaration(
    ...
    description="Runs the command on a given file, and returns a string depending on outcome. The function can take additional arguments as well.",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "file_path": types.Schema(
                type=types.Type.STRING,
                description="file path to run the command on, relative to the working directory (default is the working directory itself).",
            ),
            "args": types.Schema(
                ...
                description="additional arguments to pass to the function. By default, its None.",
            ),
        },
        required=["file_path"]
    ),
